...
void Parser::S()
{
    try
    {
        ...
        {   
            ...
            {
                cout << "\x1b[37;41m" << "Line: " << scan.count_str << " Column: " << scan.count_col << " Unexpected symbol " << c << " \n Wrong rule for WRITE " << "\x1b[0m" << endl;
                err_parser = true;
                while (c != '\"' && c != ';' && c != EOF && c != ')')
                {
                    c = scan.gc();
        ...
        else if(c_type == LEX_MINUS)
        {
            try
            {
                ...
                if(c_type == LEX_ID)
                {
                    ...
                    Eor();
                    eq_type();
                    //prog.push_back(Lex(LEX_MINUS, 0));
                    gl();
                }
            }
            catch(Lex l)
...
void Parser::E1()
{
    ...
    while (c_type == LEX_PLUS || c_type == LEX_MINUS || c_type == LEX_OR)
    {
        ...
        if (c_type == LEX_MINUS || c_type == LEX_PLUS)
        {
            ...
            {
                prog.push_back(Lex(LEX_NUM, 0));
                //prog.push_back(Lex(LEX_NUM, c_val));
                F();
                prog.push_back(Lex(tmp, 0));
            }
            else
